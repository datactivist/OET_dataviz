...
table <- reactive({
  ...
  mutate(nb_circuits_100_mars25 = nb_circuits / circuits_mars25()$amount * 100,
         nb_circuits_100_diff = round(nb_circuits_100_mars25 - 100, 1),
         nb_circuits_diff = nb_circuits - circuits_mars25()$amount,
         nb_segments_100_mars25 = nb_segments / line_mars25()$amount * 100, #amount
         nb_segments_100_diff = round(nb_segments_100_mars25 - 100, 1),
         nb_segments_diff = round(nb_segments - line_mars25()$amount, 0), #amount
...
min_mesures <- reactive({
  c(min(table()$nb_circuits_100_mars25, na.rm = TRUE),
...
max_mesures <- reactive({
  c(max(table()$nb_circuits_100_mars25, na.rm = TRUE),
...
growth_rate <- reactive({
  ...
  mutate(Mesure = gsub("^([[:alpha:]])(.*)$", "\\U\\1\\L\\2", Mesure, perl = TRUE)) #upper first letter
})
```
...
# Pylons (supports)
value_world_supports_length <- reactive({
  read_csv("./data/api/data_supports_all.csv", show_col_types = FALSE) |> 
    filter(Pays == input$nom_pays_length)
})
value_world_supports_last_length <- reactive({
  value_world_supports_length() |> 
    arrange(x) |> 
    slice_tail(n = 1) |> 
    mutate(amount = format(as.integer(amount, 0), nsmall = 1, big.mark = ","))
})
...
output$n2_length <- renderText({
  value_world_lines_last_length()$length
})
...
table_length <- reactive({
  ...
  arrange(x) |> 
  #transformation des métriques en base 100 en mars 2025
  mutate(length_circuits_100_mars25 = length_circuits / circuits_mars25_length()$length * 100,
         length_circuits_100_diff = round(length_circuits_100_mars25 - 100, 1),
         length_circuits_diff = length_circuits - circuits_mars25_length()$length,
         length_segments_100_mars25 = length_segments / line_mars25_length()$length * 100, 
         length_segments_100_diff = round(length_segments_100_mars25 - 100, 1),
         length_segments_diff = round(length_segments - line_mars25_length()$length, 0), 
         area_substations_100_mars25 = area_substations / substations_mars25_length()$area * 100,
         area_substations_100_diff = round(area_substations_100_mars25 - 100, 1),
         area_substations_diff = area_substations - substations_mars25_length()$area)
})
  #calcul de la valeur minimale en base 100 (pour placer geom_rect)
min_mesures_length <- reactive({
  c(min(table()$length_circuits_100_mars25, na.rm = TRUE),
    min(table()$length_segments_100_mars25, na.rm = TRUE),
    min(table()$area_substations_100_mars25, na.rm = TRUE))
})
min_min_100_length <- reactive({
  min(min_mesures_length())
})
  #calcul de la valeur maximale en base 100 (pour placer geom_rect)
max_mesures_length <- reactive({
  c(max(table()$length_circuits_100_mars25, na.rm = TRUE),
    max(table()$length_segments_100_mars25, na.rm = TRUE),
    max(table()$area_substations_100_mars25, na.rm = TRUE))
})
max_max_100_length <- reactive({
  max(max_mesures_length())
})
```
