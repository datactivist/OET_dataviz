...
#       # Plotly sparkline en fond
#       tags$div(
#         style = "position: absolute; top: 0; right: 0; width: 35%; height: 100%; opacity: 0.5; z-index: 1;",
#         plotlyOutput("sparkline_plot", height = "100%", width = "100%")
#       ),
#       # Contenu principal
#       tags$div(
...
#           value_world_lines_last()$amount
#         ),
#         tags$div(
#           style = "font-size: 0.5em; margin-top: 12px; opacity: 0.9;",
#           tags$div(
#             tags$img(src = "images/myg_24h.svg", 
#                      style = "height: 25px; vertical-align: top; margin-right: 5px;"), 
...
# })
# 
# # Sparkline plotly minimaliste avec tooltips
# output$sparkline <- renderPlotly({
#   
#   if (nrow(value_world_lines()) == 0 | all(is.na(value_world_lines()$amount))) {
#     plot_ly() %>% layout(paper_bgcolor = "transparent", plot_bgcolor = "transparent")
#   } else {
#     
#     sparkline_data <- value_world_lines() %>%
#       mutate(x = as.Date(x))
#     
#     plot_ly(sparkline_data) %>%
#   add_lines(
#     x = ~x, y = ~amount,
...
output$plot5 <- renderPlotly({
  ...
  graph <- table() |> 
    ...
    scale_x_date(date_labels = "%m-%y", breaks = pretty_breaks(n = 7)) +
    scale_y_continuous(labels = scales::comma) +
    scale_color_manual(values = c("Segments" = "#036d7a", 
                                  "Circuits \nrelations" = "#ecc123",
                                  "Substations" = "#54a36a",
                                  "Pylons" = "#6d0f06")) +
    labs(x = "Time", 
...
output$plot6 <- renderPlotly({
  ...
  #plot depending on selected country
  graph <- growth_rate() |> 
    mutate(Mesure = factor(Mesure, levels = c("Pylons", "Substations", "Circuits relations", "Segments"))) |> 
    ggplot() +
...
```{r}
#| title: circuits relations length
...
output$plot5_length <- renderPlotly({
  ...
  graph <- table_length() |> 
    ...
    scale_x_date(date_labels = "%m-%y", breaks = pretty_breaks(n = 7)) +
    scale_y_continuous(labels = scales::comma) +
    scale_color_manual(values = c("Lines" = "#036d7a", 
                                  "Circuits \nrelations" = "#ecc123",
                                  "Substations" = "#54a36a")) +
    labs(x = "Time", 
         y = "Base 100 in 03-2025", 
  ...
  tags$div(tags$div(style = "font-size: 1.2em;", 
    ...
    tags$div(style = "font-size: 0.5em; margin-top: 12px; opacity: 0.9;",
             tags$div(tags$img(src = "images/myg_24h.svg",                                 
                      ...
                      if(mappers_pylons_period_last()$val > mappers_world_pylons_24H()$val) {
                        tagList(tags$span(style = "color: #90EE90;", "▲"), 
                                paste0("+", mappers_world_pylons_24H()$growth_rate, "%"))
                        } else if(mappers_pylons_period_last()$val < mappers_world_pylons_24H()$val) {
                          tagList(tags$span(style = "color: #FC4B3D;", "▼"),
                                  paste0(mappers_world_pylons_24H()$growth_rate, "%"))
                          } else {
                            tags$span(style = "color: #FFA500;", "=") 
             ...
             tags$div(style = "margin-top: 2px;",
                      ...
                      if(mappers_pylons_period_last()$val > mappers_world_pylons_month1()$val) {
                        tagList(tags$span(style = "color: #90EE90;", "▲"),
                                paste0("+", mappers_world_pylons_month1()$growth_rate, "%"))
                        } else if(mappers_pylons_period_last()$val < mappers_world_pylons_month1()$val) {
                          tagList(tags$span(style = "color: #FC4B3D;", "▼"),
                                  paste0(mappers_world_pylons_month1()$growth_rate, "%"))
                          } else {
                            tags$span(style = "color: #FFA500;", "=")
...
output$plot10 <- renderPlotly({
  ...
  graph <- growth_rate_mappers() |> 
    ...
    geom_point(aes(x = Valeur, y = Mesure, color = Mesure,
                   text = paste("+", round(Valeur*100, 2), "%")), size = 4) +
    labs(y = "", x = "Growth rate (%)") +
    scale_color_manual(values = c("Circuits relations" = "#ecc123",
                                  "Segments" = "#036d7a",
                                  "Substations" = "#54a36a",
                                  "Pylons" = "#6d0f06")) +
