...
# Bouton de sélection du pays
selectInput(inputId = 'time_stat', label = NULL, choices = c("Last 24 hours", "Last 30 days"), selected = "Last 24H")
```
...
data_selected_measure_time <- reactive({
  switch(input$time_stat,
         "Last 24 hours" = data_selected_measure() |> rename(col_graph = labels.transmission.amount_1d),
         "Last 30 days" = data_selected_measure() |> rename(col_graph = labels.transmission.amount_30d))
})

# Plot users segments
output$plot7.1 <- renderPlotly({
  #plot depending on selected country
  graph <- data_selected_measure_time() |> 
    mutate(t = as.Date(t)) |> 
    ggplot() +
    #distinction des périodes du projet
    geom_rect(aes(xmin = as.Date("2024-11-01"), xmax = as.Date("2025-03-01"),
    ...
    geom_rect(aes(xmin = as.Date("2025-08-01"), xmax = max(t),
                  ...
                  text = "Public launch"), fill = "#656566", alpha = 0.2) +
    #points et lignes
    geom_line(aes(x = t, y = col_graph, group = 1,
                  text = paste0("Time : ", t, "\nNumber of mappers : ", format(as.integer(col_graph, 0), nsmall = 1, big.mark = ","))),
              color = color_selected_measure(), size = .5) +
    # geom_point(aes(x = t, y = amount, group = 1,
    #               text = paste0("Time : ", t, "\nNumber of mappers : ", format(as.integer(amount, 0), nsmall = 1, big.mark = ","))),
    #           color = color_selected_measure(), size = 1) +
    scale_y_continuous(labels = scales::comma) +
    scale_x_date(date_labels = "%m-%y", breaks = pretty_breaks(n = 10)) +
    labs(x = "Time", 
...
```{r}
#| title: Top 10 countries which received most contributions
...
# Depending on time choosed
selected_process_data_time <- reactive({
  switch(input$time_stat,
         "Last 24 hours" = selected_process_data() |> rename(col_graph = labels.transmission.amount_1d),
         "Last 30 days" = selected_process_data() |> rename(col_graph = labels.transmission.amount_30d))
})
...
output$plot7.3 <- renderPlotly({
  ...
  # Plot
  graph <- selected_process_data_time() |> 
    mutate(Pays = str_replace_all(Pays, c("United States of America" = "USA",
                                          "People's Republic of China" = "China")),
           Pays = fct_reorder(Pays, col_graph)) |> 
    ggplot() +
    geom_col(aes(x = Pays, y = max(col_graph)), fill = "#F3F3F3", width = .85) +
    geom_col(aes(x = Pays, y = col_graph,
                 text = ifelse(col_graph == 1,
                               paste(col_graph, "contribution"),
                               paste(col_graph, "contributions"))), 
