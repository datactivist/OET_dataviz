...
```{r}
#| context: server
#| label: calculate cumulative data

# Lines
mappers_lines_cumul_date <- reactive({
  mappers_lines() |> 
    mutate(amount_cumul = cumsum(amount)) |> 
    filter(Pays == input$nom_pays_mappers)
})
# Substations
mappers_substations_cumul_date <- reactive({
  mappers_substations() |> 
    mutate(amount_cumul = cumsum(amount)) |> 
    filter(Pays == input$nom_pays_mappers)
})
# Pylons
mappers_pylons_cumul_date <- reactive({
  mappers_pylons() |> 
    mutate(amount_cumul = cumsum(amount)) |> 
    filter(Pays == input$nom_pays_mappers)
})
...
output$plot7.2 <- renderPlotly({
  #plot depending on selected country
  graph <- mappers_lines_cumul_date() |> 
    mutate(t = as.Date(t)) |> 
    ggplot() +
    #distinction des périodes du projet
    geom_rect(aes(xmin = as.Date("2024-11-01"), xmax = as.Date("2025-03-01"),
                  ymin = min(amount_cumul), ymax = max(amount_cumul),
                  text = "Early OET"), fill = "#333333", alpha = 0.2) +
    geom_rect(aes(xmin = as.Date("2025-03-01"), xmax = as.Date("2025-08-01"),
                  ymin = min(amount_cumul), ymax = max(amount_cumul),
                  text = "Kickoff"), fill = "#a2a2a3", alpha = 0.2) +
    geom_rect(aes(xmin = as.Date("2025-08-01"), xmax = max(t),
                  ...
                  text = "Public launch"), fill = "#656566", alpha = 0.2) +
    #points et lignes
    geom_line(aes(x = t, y = amount_cumul, group = 1,
                  text = paste0("Time : ", t, "\nNumber of users : ", format(as.integer(amount_cumul, 0), nsmall = 1, big.mark = ","))),
              color = color_selected_measure(), size = .5) +
    # geom_point(aes(x = t, y = amount_cumul, group = 1,
    #               text = paste0("Time : ", t, "\nNumber of users : ", format(as.integer(amount_cumul, 0), nsmall = 1, big.mark = ","))),
    #           color = color_selected_measure(), size = 1) +
    scale_y_continuous(labels = scales::comma) +
    scale_x_date(date_labels = "%m-%y", breaks = pretty_breaks(n = 10)) +
