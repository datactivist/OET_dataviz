...
mappers_lines <- reactive({
  ...
  mappers_lines |> 
    ...
    rename(amount = labels.transmission.amount_1d) |> 
    filter(Pays == input$nom_pays_mappers)
})
mappers_lines_cumul <- reactive({
  mappers_lines() |> 
    summarise(somme = sum(amount, na.rm = TRUE)) |> 
    mutate(somme = format(as.integer(somme, 0), nsmall = 1, big.mark = ","))
})
# Substations
...
mappers_substations_cumul <- reactive({
  mappers_substations() |> 
    summarise(somme = sum(amount, na.rm = TRUE)) |> 
    mutate(somme = format(as.integer(somme, 0), nsmall = 1, big.mark = ","))
})
...
mappers_pylons <- reactive({
  ...
  mappers_pylons |> 
    ...
    rename(amount = amount_1d) |> 
    filter(Pays == input$nom_pays_mappers)
})
mappers_pylons_cumul <- reactive({
  mappers_pylons() |> 
    summarise(somme = sum(amount, na.rm = TRUE)) |> 
    mutate(somme = format(as.integer(somme, 0), nsmall = 1, big.mark = ","))
})
...
value_box(
  id = "card5",
  title = "Cumulative contributors for segments since 1st January of 2024",
  theme = value_box_theme(bg = "#f1d365", fg = "#4f4e4d"),
  value = textOutput("n5")
)
```
...
value_box(
  id = "card6",
  title = "Cumulative contributors for substations since 1st January of 2024",
  theme = value_box_theme(bg = "#54a36a", fg = "white"),
  value = textOutput("n6")
)
```
...
value_box(
  id = "card7",
  title = "Cumulative contributors for pylons since 1st January of 2024",
  theme = value_box_theme(bg = "#985750", fg = "white"),
  value = textOutput("n7")
)
```
...
output$n5 <- renderText({
  mappers_lines_cumul()$somme
})
output$n6 <- renderText({
  mappers_substations_cumul()$somme
})
output$n7 <- renderText({
  mappers_pylons_cumul()$somme
})
```
...
output$plot8.2 <- renderPlotly({
  #plot depending on selected country
  graph <- mappers_substations_cumul_date() |> 
    mutate(t = as.Date(t)) |> 
    ggplot() +
    #distinction des périodes du projet
    geom_rect(aes(xmin = as.Date("2024-11-01"), xmax = as.Date("2025-03-01"),
...
# Prepare data
data_graph <- mappers_substations |> 
  filter(Pays != "World (default)") |> 
  filter(t == max(t), .by = Pays) |> 
  arrange(desc(labels.transmission.amount_1d)) |> 
  filter(row_number() <= 10) |> 
...
output$plot9.2 <- renderPlotly({
  #plot depending on selected country
  graph <- mappers_pylons_cumul_date() |> 
    mutate(t = as.Date(t)) |> 
    ggplot() +
    #distinction des périodes du projet
    geom_rect(aes(xmin = as.Date("2024-11-01"), xmax = as.Date("2025-03-01"),
...
graph <- data_graph |> 
  ggplot() +
  geom_col(aes(x = Pays, y = max(amount_1d)), fill = "#F3F3F3", width = .85) +
  geom_col(aes(x = Pays, y = amount_1d,
               text = ifelse(amount_1d == 1,
                             paste(amount_1d, "contribution"),
                             paste(amount_1d, "contributions"))), 
           fill = "#6d0f06", width = .85) +
  scale_y_continuous(labels = scales::comma) + 
  coord_flip() +
  labs(y = "Number of contributions") +
  theme_custom() +
