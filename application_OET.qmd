...
mappers_pylons <- reactive({
  ...
  mappers_pylons |> 
    ...
    rename(labels.transmission.amount_1d = amount_1d,
           labels.transmission.amount = amount) |> #pas label transmission, mais on renomme pour que le graphique puisse se générer avec le nom de colonne correspondant
    filter(Pays == input$nom_pays_mappers)
})
...
mappers_jan24 <- reactive({
  switch(input$mesure,
         ...
         "Pylons" = mappers_pylons() |> arrange(desc(t)) |> 
           slice(1) |> rename(labels.transmission.amount = amount))
})
mappers_1d <- reactive({
  switch(input$mesure,
         "Segments" = mappers_lines() |> arrange(desc(t)) |> slice(1),
         "Substations" = mappers_substations() |> arrange(desc(t)) |> slice(1),
         "Pylons" = mappers_pylons() |> arrange(desc(t)) |> 
           slice(1) |> rename(labels.transmission.amount_1d = amount_1d))
})
mappers_30d <- reactive({
  switch(input$mesure,
         ...
         "Substations" = mappers_substations() |> arrange(desc(t)) |> slice(1),
         "Pylons" = mappers_pylons() |> arrange(desc(t)) |> 
           slice(1) |> rename(labels.transmission.amount_30d = amount_30d))
})
...
```{r}
#| title: Cumulative users' contributions since 2024-01-01
...
output$plot7.2 <- renderPlotly({
  ...
  graph <- data_cumul() |> 
    ...
    geom_line(aes(x = t, y = labels.transmission.amount, group = 1,
                  text = paste0("Time : ", t, "\nNumber of users : ", format(as.integer(labels.transmission.amount, 0), nsmall = 1, big.mark = ","))),
...
# Bouton de sélection du pays
selectInput('time_stat', 'Time stat', c("Last 24H", "Last 30 days"), selected = "Last 24H")
```


```{r}
#| title: Users' contributions in the last 24H
...
output$plot7.1 <- renderPlotly({
  #plot depending on selected country
  graph <- data_selected_measure() |> 
    mutate(t = as.Date(t)) |> 
    ggplot() +
    #distinction des périodes du projet
    geom_rect(aes(xmin = as.Date("2024-11-01"), xmax = as.Date("2025-03-01"),
                  ymin = min(labels.transmission.amount_1d), 
                  ymax = max(labels.transmission.amount_1d),
                  text = "Early OET"), fill = "#333333", alpha = 0.2) +
    geom_rect(aes(xmin = as.Date("2025-03-01"), xmax = as.Date("2025-08-01"),
                  ymin = min(labels.transmission.amount_1d), 
                  ymax = max(labels.transmission.amount_1d),
    ...
    geom_line(aes(x = t, y = labels.transmission.amount_1d, group = 1,
                  text = paste0("Time : ", t, "\nNumber of users : ", format(as.integer(labels.transmission.amount_1d, 0), nsmall = 1, big.mark = ","))),
              color = color_selected_measure(), size = .5) +
    # geom_point(aes(x = t, y = amount, group = 1,
    #               text = paste0("Time : ", t, "\nNumber of users : ", format(as.integer(amount, 0), nsmall = 1, big.mark = ","))),
    #           color = color_selected_measure(), size = 1) +
    scale_y_continuous(labels = scales::comma) +
    scale_x_date(date_labels = "%m-%y", breaks = pretty_breaks(n = 10)) +
    labs(x = "Time", 
...
# subplot(p1, p2, shareX = FALSE, shareY = FALSE, titleX = TRUE) %>% 
#   hide_legend() %>%
...
```{r}
#| title: Top 10 countries which received most contributions in the last 24H
...
selected_process_data <- reactive({
  switch(input$mesure,
         "Segments" = read_csv("data/api/mappers_lines_all.csv") |> 
                        ...
                        mutate(Pays = fct_reorder(Pays, labels.transmission.amount_1d)),
         ...
         "Pylons" = read_csv("data/api/mappers_supports_all.csv") |> 
                        filter(Pays != "World (default)") |> 
                        filter(t == max(t), .by = Pays) |> 
                        arrange(desc(amount_1d)) |> 
                        filter(row_number() <= 10) |> 
                        mutate(Pays = fct_reorder(Pays, amount_1d)) |> 
                        rename(labels.transmission.amount_1d = amount_1d) #pas label transmission, mais on renomme pour que le graphique puisse se générer avec le nom de colonne correspondant
         )
})

# Plot users segments
output$plot7.3 <- renderPlotly({
...
Déploiement : 
  - serveur dédié
  - peut-être pas besoin de redéployer app (juste mettre à jour les données)
```
