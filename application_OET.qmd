...
The data displayed on this dashboard are from **`r format(as.Date(first_date$t, format = "%Y-%m-%dT%H:%M:%S"),"%Y-%m-%d")`** to **`r format(as.Date(last_date$t, format = "%Y-%m-%dT%H:%M:%S"),"%Y-%m-%d %H:%M:%S")`**.
...
The data displayed on this dashboard are from **`r format(as.Date(first_date_mappers$t, format = "%Y-%m-%dT%H:%M:%S"),"%Y-%m-%d")`** to **`r format(as.Date(last_date_mappers$t, format = "%Y-%m-%dT%H:%M:%S"),"%Y-%m-%d %H:%M:%S")`**.
...
output$plot7.1 <- renderPlotly({
  ...
  graph <- mappers_lines_period() |> 
    ...
    geom_rect(aes(xmin = as.Date("2025-08-01"), xmax = max(t),
                  ...
                  text = "Public launch"), fill = "#656566", alpha = 0.2) +
    #lignes
    geom_line(aes(x = t, y = val, group = 1,
                  text = paste0("Time : ", t, "\nNumber of mappers : ", 
