...
len_mars25 <- world_line |> 
  filter(as.Date(t) == "2025-03-01")
substations_mars25 <- world_substations |> 
  filter(as.Date(t) == "2025-03-01")
pylons_mars25 <- world_supports |> 
  filter(as.Date(t) == "2025-03-01")
table <- left_join(world_line |> rename(nb_sections = amount), 
                   world_substations |> rename(nb_substations = amount) |> select(-length), 
                   by = "t") |> 
...
graph <- table |> 
  ...
  #ligne colorée jusq'à 100 pour nb of substations
  geom_line(aes(x = t, y = nb_substations_100_mars25, group = 1, 
                text = case_when(nb_substations_100_diff < 0 ~ paste0("Time : ", t, "\n↓ ", abs(nb_substations_100_diff), 
                                 ...
                                 .default = paste0("Time : ", t, "\n↑ ", abs(nb_substations_100_diff), 
                                                              "% (+", nb_substations_diff, " substations)")),
                color = "Nb. of substations"), size = .7) +
...
growth_rate <- bind_cols(
  ...
  mutate(Mesure = str_replace_all(Mesure, c("amount" = "Number of sections",
                                            "length" = "Line length",
                                            "substation" = "Number of substations",
...
```{r}
#| title: Line length's average per continent


graph <- line_lenght_country_prep2 |> 
  mutate(continent_of_the_territory = fct_reorder(continent_of_the_territory, mean_power_line)) |> 
  ggplot() +
  geom_col(aes(x = continent_of_the_territory, y = max(mean_power_line)), fill = "#F3F3F3", width = .85) +
  geom_col(aes(x = continent_of_the_territory, y = mean_power_line,
               text = paste(format(as.integer(mean_power_line, 0), nsmall = 1, big.mark = ","), "km")), 
           fill = "#4e98a1", width = .85) +
  scale_y_continuous(labels = scales::comma) + 
  coord_flip() +
  labs(y = "Line length's average (km)") +
  theme_custom() +
...
map_jan <- colombia_jan |> 
  mapview(color = "#036d7a",
          layer.name = "Grid on January 2025")
map_oct <- colombia_oct |> 
  mapview(color = "#6d0f06",
          layer.name = "Grid on October 2025")
map_jan | map_oct
```
